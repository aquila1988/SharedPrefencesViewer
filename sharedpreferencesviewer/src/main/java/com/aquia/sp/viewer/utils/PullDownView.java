...
public class PullDownView extends FrameLayout implements OnGestureListener {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (!isInit) {
			...
			addView(localView2, 0, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
			addView(localView1, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
			isInit = true;
		}
		...
		while (true) {
			...
			int measuredHeight = localView3.getMeasuredHeight();
			if (localView3.getVisibility() != View.GONE) {
				localView3.layout(0, heightPos, localView3.getMeasuredWidth(), heightPos + measuredHeight);
				heightPos += measuredHeight;
			}
			i++;
		}
	}
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		int i = -1;
		int j = 1;
		this.isMoveDown = distanceY > 0.0F;
		int k;
		...
		if (!this.mScroller.computeScrollOffset()) {
			if (this.isScrollStoped) {
				...
				if (!isRefreshData) {
					...
					this.mHandler.sendMessageDelayed(mHandler.obtainMessage(0), timeInterval);
				}
				/*if (mOnRefreshAdapterDataListener != null && !isRefreshData) {
					isRefreshData = true;
					mOnRefreshAdapterDataListener.refreshData();
				}*/
			}
